...
def Test():
    ...
    while True:
        if classname(button).text == "Sold Out":
            ...
            xpath(last).send_keys(userLast)
            xpath(street).send_keys(userAddress)
            xpath(city).send_keys(userCity)
            select = Select(xpath(state))
            select.select_by_visible_text("NY")
            xpath(zipcode).send_keys(userZipcode)
            ids(email).send_keys(userEmail)
            ids(phone).send_keys(userPhone)
            ...
            select.select_by_visible_text(userMonth)
            select = Select(xpath(xpryear))
            select.select_by_visible_text(userYear)
            ids(ccv).send_keys(userCCV)
